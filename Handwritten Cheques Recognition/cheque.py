...
def basic_ocr_to_text(image):
    ...
    cv2.waitKey(0)
    # saving image
    cv2.imwrite("/content/drive/MyDrive/Cheque_detection/image2_2.png", thresh1)
    cv2.waitKey(0)
    ...
    im = cv2.imread(
        "/content/drive/MyDrive/Cheque_detection/image2_2.png", cv2.IMREAD_COLOR
    )
...
def ext_ocr_details(img):	
	# ------ connected components ---- ###
			text = basic_ocr_to_text('/content/drive/MyDrive/Cheque_detection/Output/copped_image_for_ifsc.jpg')
			text = text.lower()
			sub = "ifs"
			ifsc = ''
			idx1 = text.index(sub)
...
def ext_amount(image, template):
	...
	amount_path = '/content/drive/MyDrive/Cheque_detection/Output'
	...
	cv2.imwrite('/content/drive/MyDrive/Cheque_detection/Output/final_templ.jpg', img_bkp)
	...
	cv2.imwrite('/content/drive/MyDrive/Cheque_detection/Output/padded_amount.jpg', padded_img)
	amount = handwriting_recognition('/content/drive/MyDrive/Cheque_detection/Output/padded_amount.jpg')
...
def handwriting_recognition(image):
    ...
    model = load_model("/content/drive/MyDrive/Dataset_google_colab/handwriting_recognition_final_model")
    image = cv2.imread(image)
...
def extract_micr (image):
	args = {
					"reference": "/content/drive/MyDrive/Cheque_detection/micr_e13b_reference (1).png"}
...
def handwritten_cheque_data_extraction(input_image):
  ...
  resized_img.save('/content/drive/MyDrive/Cheque_detection/resized_input.jpg')
  img_color = cv2.imread('/content/drive/MyDrive/Cheque_detection/resized_input.jpg')
  if img_color.ndim == 3:
      img = cv2.cvtColor(img_color, cv2.COLOR_RGB2GRAY)
  img = cv2.threshold(img, 0, 255, cv2.THRESH_OTSU)[1]
  line_corrected_img, mask = correct_line(img)
  cv2.imwrite('/content/drive/MyDrive/Cheque_detection/Output/preprocessed_img.jpg', line_corrected_img)
  numpydata = asarray(line_corrected_img)
  micr_extracted = extract_micr('/content/drive/MyDrive/Cheque_detection/resized_input.jpg')
  print('MICR->', micr_extracted)
  template = cv2.imread('/content/drive/MyDrive/Cheque_detection/rupee_template_2.jpg', 0)
  amount = ext_amount(line_corrected_img, template)
  acc_no = tr.image_to_string(line_corrected_img[550:630, 350:750])
  cv2.imwrite('/content/drive/MyDrive/Cheque_detection/Output/copped_image_for_ifsc.jpg', line_corrected_img[50:300,20:2200])
  ifsc = ext_ocr_details(line_corrected_img[50:300,20:2200])
  cv2.imwrite('/content/drive/MyDrive/Cheque_detection/Output/Payee_name.jpg', line_corrected_img[200:300,200:1800])
  payee = handwriting_recognition('/content/drive/MyDrive/Cheque_detection/Output/Payee_name.jpg')

  result = [['MICR','Amount','Account_Number','IFSC','Bearer'], [micr_extracted,amount,acc_no,ifsc,payee]]

  csv_file = "/content/drive/MyDrive/Cheque_detection/Output/Cheque_result.csv"
...
# open the file
  ...
  for page_index in range(len(pdf_file)):
      ...
      for image_index, img in enumerate(image_list, start=1):
          ...
          image.save(open(f"/content/drive/MyDrive/Cheque_detection/input_handwritten_cheques.{image_ext}", "wb"))
          handwritten_cheque_data_extraction("/content/drive/MyDrive/Cheque_detection/input_handwritten_cheques.jpeg")
